...
| D7 | **Symbols.** Members are requested as `"<ticker> US Equity"`. Bars are fetched for the **full 2010–2025 window per member** (uniform cache keys; bars remain available to manage exits after a held stock leaves the index — same unique-security cost). Ticker recycling (a delisted ticker reused by a new listing) is a known residual risk — mitigated by per-symbol coverage reporting; FIGI-based identity is deferred unless coverage stats show a real problem. *(Amended during planning: was "membership-window-only" — that fragmented cache keys and starved post-exit bars.)* |
